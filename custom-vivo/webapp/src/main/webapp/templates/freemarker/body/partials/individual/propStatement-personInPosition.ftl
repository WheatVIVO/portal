...
<#macro showPosition statement>
    ...
    <#local immediateOrganization>
        <#if statement.immediateOrgName??>
            <span itemprop="worksFor" itemscope itemtype="http://schema.org/Organization">
                ${statement.immediateOrgName!}  
            </span>
	</#if>
    </#local>
    
    <@s.join [ posTitle, linkedIndividual, immediateOrganization!, middleOrganization! ]/>  <@dt.yearIntervalSpan "${statement.dateTimeStart!}" "${statement.dateTimeEnd!}" />
